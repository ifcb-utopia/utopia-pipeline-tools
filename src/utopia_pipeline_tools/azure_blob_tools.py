...
def create_container(container_name, 
    ...
    """
    Creates a new container in the Azure Blob storage. This function has not 
    been tested. 
...
def upload_images_to_blob(container_name, local_directory_path, 
                          blob_storage_name=config_info['blob_storage_name']):
    """
    Upload a folder to the Azure Blob storage. This function has not been tested
    and likely needs edits. 
    ...
    if blob_storage_name is None:
        ...
        os.system(f"""azcopy copy '{local_directory_path}' 
                'https://{blob_storage_name}.blob.core.windows.
...
def list_files_in_blob(container, 
    ...
    if connection_string is None:
        ...
        if selection == 'png':
            ...
            csv_df = pd.DataFrame({'filepath': csv_list})

            return csv_df
        
        elif selection == 'all':
            all_df = pd.DataFrame({'filepath': filepaths})
